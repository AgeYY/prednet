...
explained_var_thre_pca_all_data = 0.90
# drifting grating configurations
label_id = (0, 1) # 0 is temporal frequency, 1 is orientation

mesh_size = 60
mesh_bound = [[0, 16], [0, 360]]
mesh_random = False
mesh_grid = True # grid mesh makes better visualization

kernel_mesh_size = 10
#kernel_mesh_bound = [[0.1, 4], [40, 80]]
## step 1: carse search using grid, single label_id
#kernel_random = False
# step 2: refine kernel_mesh_bound acording to step 1, use random search
kernel_mesh_bound = [[0.1, 1], [40, 80]]
kernel_random = True

var_period = [None, [0, 360]] # the length is equal to the number of labels (columns of train_label). None means this variable is linear, while given a period interval, von mises function would be used as a kernel
kernel_width = [0.47, 49]
n_layer = 1
layer_order = ['VISp']
