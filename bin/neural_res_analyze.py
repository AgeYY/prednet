...
for layer_name in layer_order:
    ...
    fig, ax =  plt_dr.plot_dimension_reduction(info_manifold, colorinfo=lt_mesh[label_id], mode='2D', fit=True)
    #fig, ax =  plt_dr.plot_dimension_reduction(feamap_validate[layer_name], colorinfo=label_validate[:, label_id], mode='2D', fig=fig, ax=ax, marker='+') # validation
    fig, ax =  plt_dr.plot_dimension_reduction(feamap_test[layer_name], colorinfo=label_test[:, label_id], mode='2D', fig=fig, ax=ax, marker='+') # test
    plt.show()
############################## Tune the kernel_width
